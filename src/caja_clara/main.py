    ...
    try:
        for msg in client.fetch_unseen():
            ...
            try:
                ...
                if extract:
                    # Deduplicación por attachment_hash
                    duplicate = False
                    if extract.attachment_hash:
                        existing = db_session.query(InvoiceRecord).filter_by(attachment_hash=extract.attachment_hash).first()
                        if existing:
                            duplicate = True
